...
function fetch(request: { url: string; method: string }): [];
function fetch(
...
type Get = {
  (request: string): [];
  (request: string, onSent: () => void): [];
};
const get: Get = (request: string, onSent?: () => void) => {
