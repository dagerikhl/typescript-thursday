...
const getEntities = (
  entityTargetOrTargets: string | string[],
  nameRegEx?: RegExp,
  limit?: number
): Entity[] => {
  let allEntities: Entity[] = [];
  if (Array.isArray(entityTargetOrTargets)) {
    for (const entityTarget of entityTargetOrTargets) {
      allEntities.push(...ENTITIES[entityTarget]);
    }
  } else {
    allEntities.push(...ENTITIES[entityTargetOrTargets]);
  }

  if (nameRegEx) {
    allEntities = allEntities.filter(({ name }) => nameRegEx.test(name));
  }

  if (typeof limit === "number") {
    allEntities = allEntities.slice(0, limit);
  }
