...
const isResponse = (value: any): value is Response => {
  const hasTwoProperties = Object.keys(value).length === 2;
  const hasCorrectDataProperty = "data" in value && Array.isArray(value.data);
  const hasCorrectTotalPropert =
    "total" in value && typeof value.total === "number";

  return hasTwoProperties && hasCorrectDataProperty && hasCorrectTotalPropert;
};
